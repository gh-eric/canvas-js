...
var api=window.location.origin+"/api/v1/";$(ta_todo());$(add_people_functions());$(hide_sidebars());$(add_profile_grades());$(add_profile_file_quota_status());var sleep=function(a){return new Promise(function(b){return setTimeout(b,a)})};function is_todo_page(){return"/"==window.location.pathname||is_course_page()}function is_course_page(){var a;return null!=(a=/\/courses\/[0-9]*$/.test(window.location.pathname))?a:!1}
function is_course_user_page(){var a;return null!=(a=/\/courses\/[0-9]+\/users$/.test(window.location.pathname))?a:!1}function is_calendar_page(){var a;return null!=(a=/\/calendar.*$/.test(window.location.pathname))?a:!1}function get_course_from_uri(a){return(a=a.match(".*/courses/([0-9]+).*"))?a[1]:null}function is_student_or_observer(){var a,b;return!(null==(a=window.ENV)?0:a.current_user_roles.includes("teacher"))&&!(null==(b=window.ENV)?0:b.current_user_roles.includes("admin"))}
function is_user_profile_page(){var a;return null!=(a=/\/users\/[0-9]+$/.test(window.location.pathname))?a:!1}function is_user_profile_settings_page(){var a;return null!=(a=/\/profile\/settings$/.test(window.location.pathname))?a:!1}function get_user_from_profile_uri(){return is_user_profile_page()?window.location.pathname.split("/").reverse()[0]:0}function get_cal_event_id(a){}
...
function add_handler_for_student_list(){if(!added_student_list){ul_class="al-options ui-menu ui-widget ui-widget-content";var a=document.getElementsByClassName(ul_class)[0],b=document.createElement("li");b.setAttribute("role","presentation");b.setAttribute("class","ui-menu-item");var d=document.createElement("a"),k=document.createTextNode(" Display Student List");d.appendChild(k);d.title="Display Student List";d.href="#";d.setAttribute("class","ui-corner-all");d.setAttribute("tabindex","-1");d.setAttribute("role",
...
function add_handler_for_export(){if(!added_handler_for_export){ul_class="al-options ui-menu ui-widget ui-widget-content";var a=document.getElementsByClassName(ul_class)[0],b=document.createElement("li");b.setAttribute("role","presentation");b.setAttribute("class","ui-menu-item");var d=document.createElement("a"),k=document.createTextNode(" Export Visible");d.appendChild(k);d.title="Export Visible";d.href="#";d.setAttribute("class","ui-corner-all");d.setAttribute("tabindex","-1");d.setAttribute("role",
"menuitem");k=document.createElement("i");k.setAttribute("class","icon-archive");d.prepend(k);b.appendChild(d);a.appendChild(b);b.addEventListener?b.addEventListener("click",export_visible_list,!1):b.attachEvent("onclick",export_visible_list);added_handler_for_export=!0}}
function hide_sidebars(){if(is_student_or_observer()){is_calendar_page()||is_user_profile_settings_page()||$("#right-side-wrapper").hide();$("#left-side").hide();$("#courseMenuToggle").hide();$("div.ic-Layout-columns").css("marginLeft","0px");var a=$("a.grades");a.addClass("btn button-sidebar-wide");$("#breadcrumbs").append(a)}}
